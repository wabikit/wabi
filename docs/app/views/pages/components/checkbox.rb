  ...
  module Pages
    module Components
      class Checkbox < Views::Base
        SOURCE_PATHS = %w[app/components/ui/checkbox.rb].freeze

        def view_template
          render ::Components::Site::Layout.new(title: "Checkbox", chrome: :full) do
            main(class: "container mx-auto py-12 px-4 max-w-3xl") do
              p(class: "text-sm text-muted-foreground mb-2") do
                a(href: "/docs/components", class: "hover:text-foreground") { "← Components" }
              end
              h1(class: "text-4xl font-bold mb-2") { "Checkbox" }
              ...
              ul(class: "list-disc pl-5 space-y-1 text-sm text-muted-foreground") do
                li { 'role="checkbox" + aria-checked reflecting current state.' }
                li { "Native focus-visible:ring keeps focus state visible without click pollution." }
                li { "Space toggles state; Tab moves to next focusable." }
                li { "When inside a <label>, clicking the label text toggles the checkbox via Zag's label wiring." }
              end
            end
          end
        end
        ...
        def description
          @description ||= YAML.safe_load_file(
            Rails.root.join("..", "registry", "components", "checkbox", "manifest.yml").realpath
          )["description"]
        end
      end
    end
